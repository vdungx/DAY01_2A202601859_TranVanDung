...
def run_assistant(
    ...
    while True:
        ...
        stats = estimate_cost(user_msg, reply, model=OPENAI_MODEL)
...
def format_comparison_table(results: list[dict]) -> str:
    def shorten(text: str) -> str:
        ...
        return text_clean if len(text_clean) <= 40 else text_clean[:40] + "..."

    headers = ["Prompt", "GPT-4o Response", "Mini Response", "GPT-4o Latency", "Mini Latency"]
    col_widths = [45, 45, 45, 16, 16]
    ...
    for res in results:
        ...
        row = f"{p:<45} | {r4o:<45} | {rmini:<45} | {l4o:<16} | {lmini:<16}"
        rows.append(row)
